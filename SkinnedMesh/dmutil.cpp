...
HRESULT CMusicManager::Initialize( HWND hWnd, DWORD dwPChannels, DWORD dwDefaultPathType, LPDIRECTSOUND pDS )
{
    ...
    if( FAILED( hr = m_pPerformance->InitAudio( NULL,
												ppDirectSound,
												hWnd,
												dwDefaultPathType, 
                                                dwPChannels,
    ...
    if( m_p3DAudioPath ) // might be NULL if dwDefaultPathType == 0
    {

		//get listener
        if( SUCCEEDED( hr = m_p3DAudioPath->GetObjectInPath( 0,
                                                ...
                                                IDirectMusicAudioPath8* p3DAudioPath )
{
	...
	if( *pp3DMusicSegment != NULL ) SAFE_DELETE(*pp3DMusicSegment);
...
//-----------------------------------------------------------------------------
/*
VOID CMusicManager::Set3DParameters(  D3DVECTOR  vPosition, float fMinDistance, float fMaxDistance )
{
		
    m_dsBufferParams.flMinDistance = fMinDistance;
    m_dsBufferParams.flMaxDistance = fMaxDistance;
	m_dsBufferParams.vPosition = vPosition;
...
HRESULT CMusicSegment::Play( DWORD dwFlags, IDirectMusicAudioPath8* pAudioPath )
{
    ...
    return m_pPerformance->PlaySegmentEx( m_pSegment,
										  0,
										  NULL,
										  dwFlags, 
                                          0,
...
// Desc: 
//-----------------------------------------------------------------------------
VOID C3DMusicSegment::SetObjectProperties( D3DXVECTOR3* pv3Position )
{
	D3DVECTOR* pvPosition = (D3DVECTOR*)pv3Position;
    DWORD dwApplyFlag = ( m_bDeferSettings ) ? DS3D_DEFERRED : DS3D_IMMEDIATE;
