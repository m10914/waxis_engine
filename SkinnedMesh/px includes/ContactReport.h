...
class TriggerCallback : public NxUserTriggerReport
{
	...
	{
		//HTMLLog("triggo!\n");
		CModelUserData* susrdata = (CModelUserData*)otherShape.getActor().userData;
		if( !susrdata ) return;
		CModelUserData* tusrdata = (CModelUserData*)triggerShape.getActor().userData;
		if( !tusrdata ) return;

		if( susrdata->type == UI_SKINMESH && tusrdata->type == UI_WEAPON
			&& otherShape.getGroup() == GROUP_BIPED
			&& *(tusrdata->attacherid) != susrdata->id )
		{
			//HTMLLog("%d %d\n", (int)otherShape.getGroup(), (int)triggerShape.getGroup());
			//if( tusrdata->attacherid != susrdata->id ) HTMLLog("%d %d\n", tusrdata->attacherid, susrdata->id);
			susrdata->bHit = TRUE;
			susrdata->targetid = tusrdata->id;
			susrdata->targettype = tusrdata->type;
		}
